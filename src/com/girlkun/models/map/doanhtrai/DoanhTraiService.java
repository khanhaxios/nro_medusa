...
public class DoanhTraiService {
    ...
    private static boolean resetDoneToday = false;
    ...
    public static final byte TIME_DELAY_DT_SECOND = 5;
    private int day = -1;
    ...
    public void update(Player player) {
        ...
        if (player.isPl()
            ...
            try {
                ...
                if (now > TIME_RESET_DT && now < TIME_DELAY_DT && !resetDoneToday) {
                    if (player.clan.doanhTrai != null) {
                        player.clan.doanhTrai = null;
                        player.clan.doanhTrai_haveGone = false;
                    }
                    resetDoneToday = true; // đánh dấu đã reset
                }
                // Khi sang ngày mới thì reset lại flag
                if (DoanhTraiService.I.day != TimeUtil.getCurrDay()) {
