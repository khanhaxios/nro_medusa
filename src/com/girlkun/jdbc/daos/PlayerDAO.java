...
// data lks
                ...
                String query = "update player set dk_kethon = ?, ket_hon = ?,dk_tutien = ?, Tu_tien = ?,dhtime = ?, dhtime2 = ?, dhtime3 = ?, dhieu = ?,head = ?, gender = ?, have_tennis_space_ship = ?," + "clan_id_sv1 = ?, data_inventory = ?, data_location = ?, data_point = ?, data_magic_tree = ?," + "items_body = ?, items_bag = ?, items_box = ?, items_box_lucky_round = ?, friends = ?," + "enemies = ?, data_intrinsic = ?, data_item_time = ?,data_item_time_sieucap = ?, data_task = ?, data_mabu_egg = ?, data_dua = ?, Tai_xiu = ?, pet = ?, dao_lu = ?," + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, violate=?, pointPvp=?,info_phoban =?, info_achievement =?," + " Thu_TrieuHoi= ?,  nhiemvu_chienthan= ?, NguHanhSonPoint=?, data_card=?, data_lks=? ,tien_luc=? , tl_dl=?,lucky_pool=?,den_bu  = ? where id = ?";
                GirlkunDB.executeUpdate(query, dk_kethon, kethon, dk_tutien, TuTien, dhtime, dhtime2, dhtime3, title, player.head, player.gender, player.haveTennisSpaceShip, (player.clan != null ? player.clan.id : -1), inventory, location, point, magicTree, itemsBody, itemsBag, itemsBox, itemsBoxLuckyRound, friend, enemy, intrinsic, itemTime, itemTimesieucap, task, mabuEgg, timedua, taixiu, pet, petDaoLu, dataBlackBall, sideTask, charm, skills, skillShortcut, player.diemdanh, pointPvp, info_phoban, info_achive, Thu_TrieuHoi, chienthan, player.NguHanhSonPoint, JSONValue.toJSONString(player.Cards), luyenKhiSu, player.tienLuc, player.tyLeTangPhamDaoLu, dataLuckyP, player.isNhanQuaDenBu, player.id);
