        ...
        try {
            ...
            switch (status) {
                ...
                case ATTACK:
                    ...
                    if (bossAttack != null || mobAttack != null) {
                        if (useSkill3()
                                || useSkill4()
                                || useSkill5(bossAttack, mobAttack)) {
                            break;
                        }
...
//            Logger.logException(Pet.class, e);
    ...
    private void useSkill1_2(Player plTarget, Mob mobTarget) {
        if (plTarget != null) {
            disToTarget = Util.getDistance(this, plTarget);
        } else if (mobTarget != null) {
            disToTarget = Util.getDistance(this, mobTarget);
        }
        ...
        if (disToTarget <= ARANGE_ATT_SKILL1) {
            this.playerSkill.skillSelect = getSkill(1);
            if (SkillService.gI().canUseSkillWithCooldown(this)) {
                if (SkillService.gI().canUseSkillWithMana(this)) {
                    ...
                    if (plTarget != null) {
                        PlayerService.gI().playerMove(this,
                                plTarget.location.x + Util.nextInt(-20, 20), plTarget.location.y);
                    } else if (mobTarget != null) {
                        PlayerService.gI().playerMove(this,
                                mobTarget.location.x + Util.nextInt(-20, 20), mobTarget.location.y);
                    }

                    SkillService.gI().useSkill(this, plTarget, mobTarget, null);
                } else {
                    askPea();
                }
            }
            ...
            if (this.playerSkill.skillSelect.skillId != -1) {
                ...
                if (SkillService.gI().canUseSkillWithCooldown(this)) {
                    if (SkillService.gI().canUseSkillWithMana(this)) {
                        if (plTarget != null) {
                            PlayerService.gI().playerMove(this,
                                    plTarget.location.x + Util.nextInt(-20, 20),
                                    plTarget.location.y);
                        } else if (mobTarget != null) {
                            PlayerService.gI().playerMove(this,
                                    mobTarget.location.x + Util.nextInt(-20, 20), mobTarget.location.y);
    ...
    private boolean useSkill3() {
        try {
            ...
            if (!Util.canDoWithTime(this.playerSkill.lastTimePrepareQCKK, TIME_WAIT_SKILL_5 + 1000)) {
                return false;
            }
    ...
    private boolean useSkill5(Player plTarget, Mob mobTarget) {
        try {
            ...
            switch (this.playerSkill.skillSelect.template.id) {
                ...
                case Skill.QUA_CAU_KENH_KHI:
                    if (!Util.canDoWithTime(this.playerSkill.lastTimePrepareQCKK, TIME_WAIT_SKILL_5)) {
                        return true;
                    }
                    if (SkillService.gI().canUseSkillWithCooldown(this)
                            && SkillService.gI().canUseSkillWithMana(this)) {
                        if (Util.canDoWithTime(this.playerSkill.lastTimePrepareQCKK,
                                TIME_WAIT_SKILL_5 + 3000)) {
                            petTalkWhenUseSkill();
                        }
                        SkillService.gI().useSkill(this, plTarget, mobTarget, null);
                        return true;
    ...
    private void petTalkWhenUseSkill() {
        String petTalkText = "";
        switch (this.playerSkill.skillSelect.template.id) {
            case Skill.THAI_DUONG_HA_SAN:
                petTalkText = "Bất ngờ chưa con nghiện";
                break;
            case Skill.TAI_TAO_NANG_LUONG:
                petTalkText = "Ta sẽ nghiêm túc đánh nhau với ngươi!";
                break;
            case Skill.KAIOKEN:
                if (Util.isTrue(20, 100)) {
                    switch (Util.nextInt(1, 6)) {
                        case 1:
                            petTalkText = "Kaiokennnnn!";
                            break;
                        case 2:
                            petTalkText = "Cảm thấy sức mạnh tràn trề trong cơ thể ta!";
                            break;
                        case 3:
                            petTalkText = "Cơ thể này đang dần bị kaioken hao mòn...";
                            break;
                        case 4:
                            petTalkText = "Kaioken x50";
                            break;
                        case 5:
                            petTalkText = "Kaioken x100";
                            break;
                        case 6:
                            petTalkText = "Kaioken x200";
                            break;
                    }
                }
                break;
            case Skill.BIEN_KHI:
                petTalkText = "Hãy ném thử sức mạnh hóa Khỉ của tộc người Xayda điii!";
                break;
            case Skill.KHIEN_NANG_LUONG:
                petTalkText = "Đến đấm bố mày hộ cái";
                break;
            case Skill.DE_TRUNG:
                petTalkText = "Triệu hồi quỷ chim, tấn công cho ta";
                break;
            case Skill.DICH_CHUYEN_TUC_THOI:
                petTalkText = "Dịch chuyển tức thời!";
                break;
            case Skill.THOI_MIEN:
                petTalkText = "Đứng im!";
                break;
            case Skill.QUA_CAU_KENH_KHI:
                petTalkText = "Cho ta mượn một ít sức mạnh của tất cả mọi người!";
                break;
            case Skill.SUPER_KAME:
                petTalkText = "Kameehamee Haaaaaa!";
                break;
            case Skill.MA_PHONG_BA:
                petTalkText = "Chịu Chết Điiiii!";
                break;
            case Skill.LIEN_HOAN_CHUONG:
                petTalkText = "Đỡ Lấy Liên Hoàn Chưởng Của Taa!";
                break;
        }
        if (petTalkText != "") {
            Service.getInstance().chat(this, petTalkText);
        }
    }
    ...
    private void updatePower() {
        if (this.playerSkill != null) {
            switch (this.playerSkill.getSizeSkill()) {
                ...
                //case 4:
                case 4:
                    if (this.nPoint.power >= 300000000000L) {
                        openSkill5();
                    }
                    break;
            }
...
//    ========================================================
    ...
    public void transform() {
        ...
        if (this.typePet == 4) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "Tao là VUA bigbang");
        }
        if (this.typePet == 5) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "KAME KAME KAME HAAAAAAAAAA");
        }
        if (this.typePet == 6) {
            this.isTransform = !this.isTransform;
            Service.getInstance().Send_Caitrang(this);
            Service.getInstance().chat(this, "FUSION HAAAAAAAAAAAAAAAAA");
        }
    }
