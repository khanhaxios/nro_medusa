...
public class EffectSkin {
    ...
    public void update() {
        ...
        if (!this.player.isBoss && !this.player.isPet && !this.player.isDaoLu && !player.isNewPet && !this.player.isTrieuhoipet) {
            updateTrainArmor();
        }
    ...
    private void updateCTHaiTac() {
        if (this.player.setClothes.ctHaiTac != -1
            ...
            for (Player pl : players) {
                ...
                if (!pl.isPet && !pl.isDaoLu && !pl.isNewPet && !pl.isTrieuhoipet && Util.canDoWithTime(lastTimeUpdateCTHT, 5000)) {
