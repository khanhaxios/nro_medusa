...
public class RewardService {
    ...
    public List<Item> getListItemLuckyRoundSpecial(Player player, int num) {
        List<Item> list = new ArrayList<>();
        for (int i = 0; i < num; i++) {
            ItemLuckyRound item = Manager.LUCKY_ROUND_REWARDS_SPECIAL.get(Util.nextInt(0, Manager.LUCKY_ROUND_REWARDS_SPECIAL.size() - 1));
            if (item.temp.gender == player.gender || item.temp.gender > 2) {
                if (Util.isTrue(item.ratio, item.typeRatio)) {
                    Item it = ItemService.gI().createNewItem(item.temp.id);
                    for (ItemOptionLuckyRound io : item.itemOptions) {
