...
//    }
    ...
    public void setNotBienhinh2(Player player) {
        Message msg;
        try {
            msg = new Message(-90);
    ...
    public void showListTop(Player player, List<TOP> tops) {
        ...
        try {
            ...
            if (tops == Manager.topSD) {
                ...
                msg.writer().writeUTF("Top Sự Kiện Hè Medusa");
            } else if (tops == Manager.topSKSanMa) {
                msg.writer().writeUTF("Top Sự Kiện Săn Ma");
            } else if (tops == Manager.topSK2T9) {
                msg.writer().writeUTF("Top Sự Kiện 2 Tháng 9");
            } else if (tops == Manager.topSKTrungThu) {
                msg.writer().writeUTF("Top Sự Kiện Trung Thu");
            } else if (tops == Manager.topVND) {
                msg.writer().writeUTF("Top Nạp");
            } else if (tops == Manager.topDaoLu) {
                msg.writer().writeUTF("Top Cảnh Giới Đạo Lữ");
            } else {
            ...
            for (int i = 0; i < tops.size(); i++) {
                TOP top = tops.get(i);
                Player pl;
                if (tops == Manager.topDaoLu) {
                    pl = GodGK.loadById(top.getId_player()).petDaoLu;
                    msg.writer().writeInt(i + 1);
                    msg.writer().writeInt((int) pl.getMaster().id);
                    ...
                    String nameDaoLuTop = "[Đạo Lữ của " + pl.getMaster().name + "]" + ((DaoLu) pl).nameDaoLu;
                    msg.writer().writeUTF(nameDaoLuTop);
...
//                msg.writer().writeUTF(isPVP == 1 ? top.getInfo2() : top.getInfo2() + pl.numKillSieuHang);
    ...
    public void sendMessAllPlayerInMap(Player player, Message msg) {
        ...
        if (MapService.gI().isMapOffline(player.zone.map.mapId)) {
            if (!player.isPl()) {
                player.getMaster().sendMessage(msg);
            } else {
                player.sendMessage(msg);
            }
        } else {
    ...
    public static void loginDe(int status) {
...
//            String freePhysicalMemoryStr = decimalFormat.format((double) freePhysicalMemorySize / (1024 * 1024 * 1024));
            if (text.equals("admin")) {
                ...
                byte nrsd = Byte.parseByte(text.replaceAll("nrsd", "").trim());
                if (nrsd >= 1 && nrsd <= 7) {
                    player.rewardBlackBall.reward(nrsd);
                    sendThongBao(player, "Admin check NRSD: " + nrsd);
                } else {
                    sendThongBao(player, "Admin check NRSD: Không hợp lệ!!!");
                }
                return;
            } else if (text.startsWith("uppp")) {
                if (player.petDaoLu != null) {
                    try {
                        long power = Long.parseLong(text.replaceAll("uppp", ""));
                        addSMTN(player.petDaoLu, (byte) 2, power, false);
                        return;
                    } catch (Exception e) {
                ...
                InventoryServiceNew.gI().addItemBag(player, item);
                InventoryServiceNew.gI().sendItemBags(player);
                Service.gI().sendThongBao(player, "Đã lấy vật phẩm: " + item.template.name + " Số lượng: " + quantity + " ra từ kho đồ vũ trụ!");
...
//                }
        ...
        // Player
        if (text.equals("tab")) {
            NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_USER_TAB_PET, -1, "|1|Chọn Loại Thông Tin Sẽ Hiển Thị Trong Phần Đề Tử\n", "Tab\nĐệ Tử", "Tab\nĐạo Lữ");
            return;
        }
        if (text.equals("dl") || text.equals("daolu")) {
            infoDaoLu(player);
        ...
        if (text.startsWith("ten con la ")) {
            ...
            Service.getInstance().sendThongBao(player, "HP bản thân:\b|7|" + Util.powerToStringnew(player.nPoint.hp) + "/" + Util.powerToStringnew(player.nPoint.hpMax) + "\b|8|KI bản thân:\b|5|" + Util.powerToStringnew(player.nPoint.mp) + "/" + Util.powerToStringnew(player.nPoint.mpMax) + "\b|8|Sức đánh:\b|4|" + Util.powerToStringnew(player.nPoint.dame) + "\b|8|Giáp:\b|1|" + Util.powerToStringnew(player.nPoint.def) + player.Hppl);
            ...
            if (player.muanhieu == false) {
                ...
                Service.gI().sendThongBao(player, "|7|Đã Dừng Auto Mua số lượng lớn");
...
//            }
    ...
    public void ChangeTabPet(Player player, byte TypeTab) {
        switch (TypeTab) {
            case 0: //Đệ Tử
                ...
                if (player.pet != null && player.typeTabPet != TypeTab) {
            ...
            case 1: // Đạo Lữ
                if (player.petDaoLu == null) {
                    Service.getInstance().sendThongBao(player, "Bạn chưa có đạo lữ để chuyển tab!");
                }
                if (player.petDaoLu != null && player.typeTabPet != 1) {
                    Service.getInstance().sendThongBao(player, "Đã chuyển tab sang Đạo Lữ");
                    player.typeTabPet = 1;
                }
                break;
            default:
    ...
    public void point(Player player) {
        ...
        Send_Info_NV(player);
        if (!player.isPet && !player.isDaoLu && !player.isBoss && !player.isNewPet && !player.isTrieuhoipet) {
            Message msg;
            try {
                msg = new Message(-42);
...
//            }
    ...
    public void addSMTN(Player player, byte type, double param, boolean isOri) {
        if (player.isPet || player.isTrieuhoipet || player.isDaoLu) {
...
//    }
    ...
    public void hsChar(Player pl, double hp, double mp) {
        ...
        try {
            ...
            pl.nPoint.setHp(hp);
            pl.nPoint.setMp(mp);
            if (!pl.isPet && !pl.isDaoLu && !pl.isNewPet && !pl.isTrieuhoipet) {
                msg = new Message(-16);
                pl.sendMessage(msg);
                msg.cleanup();
    ...
    public void charDie(Player pl) {
        ...
        try {
            if (!pl.isPet && !pl.isDaoLu && !pl.isNewPet && !pl.isTrieuhoipet) {
                ...
                ((DaoLu) pl).lastTimeDie = System.currentTimeMillis();
            ...
            if (!pl.isPet && !pl.isDaoLu && !pl.isBoss && pl.idNRNM != -1) {
...
//            sendMessAllPlayerInMap(zone, msg);
    ...
    public void InfoDaoLuGoc(Player pl) {
        if (pl != null && pl.petDaoLu != null) {
            Message msg;
            try {
                msg = new Message(-109);
    ...
    public void showInfoDaoLu(Player pl) {
        if (pl != null && pl.petDaoLu != null) {
            Message msg;
            try {
                msg = new Message(-107);
                msg.writer().writeByte(2);
                msg.writer().writeShort(pl.petDaoLu.getAvatar());

                msg.writer().writeByte(pl.petDaoLu.inventory.itemsBody.size());

                for (Item item : pl.petDaoLu.inventory.itemsBody) {
                    if (!item.isNotNullItem()) {
                        msg.writer().writeShort(-1);
                    } else {
                        msg.writer().writeShort(item.template.id);
                ...
                msg.writer().writeLong(pl.petDaoLu.nPoint.power); //power
                msg.writer().writeLong(pl.petDaoLu.nPoint.tiemNang); //tiềm năng
                ...
                for (int i = 0; i < pl.pet.playerSkill.skills.size(); i++) {
                    if (pl.pet.playerSkill.skills.get(i).skillId != -1) {
                        ...
                        switch (i) {
                            ...
                            default:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Cần đạt sức mạnh 300tỷ\nđể mở");
                                break;
                        }
                    }
                }
        ...
        if (player.getSession().pp.equals(oldPass)) {
            if (newPass.length() >= 6) {
                if (newPass.equals(rePass)) {
                    ...
                    try {
                        GirlkunDB.executeUpdate("update account set password = ? where id = ? and username = ?", rePass, player.getSession().userId, player.getSession().uu);
