...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final List<ItemLuckyRound> LUCKY_ROUND_REWARDS_SPECIAL = new ArrayList();
    ...
    public static final String QUERY_TOP_SK_2T9 = "SELECT id, eventColumn FROM (SELECT player.id, CAST(JSON_EXTRACT(data_inventory, '$[6]') AS UNSIGNED) AS eventColumn FROM player JOIN account ON player.account_id = account.id WHERE account.id NOT IN (SELECT id FROM account WHERE is_admin = 1)) AS subquery WHERE eventColumn IS NOT NULL ORDER BY eventColumn DESC LIMIT 20;";
    public static final String QUERY_TOP_SK_TRUNG_THU = "SELECT id, eventColumn FROM (SELECT player.id, CAST(JSON_EXTRACT(data_inventory, '$[7]') AS UNSIGNED) AS eventColumn FROM player JOIN account ON player.account_id = account.id WHERE account.id NOT IN (SELECT id FROM account WHERE is_admin = 1)) AS subquery WHERE eventColumn IS NOT NULL ORDER BY eventColumn DESC LIMIT 20;";
    ...
    public static List<TOP> topSKSanMa;
    public static List<TOP> topSK2T9;
    public static List<TOP> topSKTrungThu;
    ...
    public static final short[] DoThanhTon = {1431, 1432, 1433, 1434, 1435};
    public static final short[] DoNguyenThuy = {1450, 1451, 1452, 1453, 1454};
    public static final short[] DoThongKho = {1482, 1483, 1484, 1485, 1486};

    public static final short[][] doThienSuSKH = {{1048, 1051, 1054, 1057, 1060}, {1049, 1052, 1055, 1058, 1061}, {1050, 1053, 1056, 1059, 1062}};
    public static final short[] setSen = {1389, 1390, 1391, 1392, 1393};
            ...
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        LUCKY_ROUND_REWARDS_SPECIAL.add(item);
                    }
                }
            }
...
//                    dataArray.clear();
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    ...
                    case queryTopSD:
                        ...
                        break;
                    case QUERY_TOP_DAO_LU:
                        top.setInfo1("Đạo Lữ - " + DaoLu.sGetTypeString(rs.getInt("pham")));
                        top.setInfo2(DaoLu.sGetCapBacCapTinh(rs.getInt("canh_gioi"), rs.getInt("canh_tinh")));
                        break;
                }
                tops.add(top);
            }
    ...
    public void loadProperties() throws IOException {
        ...
        if ((value = properties.get("server.girlkun.vsClient")) != null) {
            VERSION_CLIENT = String.valueOf(value).split(":");
            DataGame.vsData = Byte.parseByte(VERSION_CLIENT[0]);
            DataGame.vsMap = Byte.parseByte(VERSION_CLIENT[1]);
            DataGame.vsSkill = Byte.parseByte(VERSION_CLIENT[2]);
            DataGame.vsItem = Byte.parseByte(VERSION_CLIENT[3]);
            DataGame.vsRes = Integer.parseInt(VERSION_CLIENT[4]);
        }
