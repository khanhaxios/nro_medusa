...
//            msg.writer().writeUTF(player.playerTask.taskMain.name);
    ...
    public void sendNextTaskMain(Player player) {
        ...
        sendTaskMain(player);
        Service.getInstance().sendThongBao(player, "Nhiệm vụ tiếp theo của bạn là " + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).name);
    }
    ...
    public void sendInfoCurrentTask(Player player) {
        Service.getInstance().sendThongBao(player, "Nhiệm vụ hiện tại của bạn là " + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).name);
    }
    ...
    public boolean checkDoneTaskTalkNpc(Player player, Npc npc) {
        switch (npc.tempId) {
            ...
            case ConstNpc.BUNMA_TL:
                return (doneTask(player, ConstTask.TASK_22_0) || doneTask(player, ConstTask.TASK_22_4) || doneTask(player, ConstTask.TASK_23_4) || doneTask(player, ConstTask.TASK_24_4) || doneTask(player, ConstTask.TASK_25_5) || doneTask(player, ConstTask.TASK_26_4) || doneTask(player, ConstTask.TASK_27_4) || doneTask(player, ConstTask.TASK_28_5) || doneTask(player, ConstTask.TASK_31_4) || doneTask(player, ConstTask.TASK_29_4) || doneTask(player, ConstTask.TASK_30_5));
...
//                    break; 
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_1_0:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.getInstance().sendThongBao(player, "Bạn đánh được " + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/" + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount + " mộc nhân");
                    }
                    break;
                ...
                case ConstTask.TASK_4_0:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.getInstance().sendThongBao(player, "Bạn đánh được " + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/" + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount + transformName(player, " %4 mẹ"));
                    }
                    break;
                ...
                case ConstTask.TASK_20_6:
                    npcSay(player, ConstNpc.CUI, "Ngọc Rồng Medusa");
                    break;
                //--------------------------------------------------------------
                ...
                case ConstTask.TASK_22_0:
                    npcSay(player, ConstNpc.BUNMA_TL, "Ngọc Rồng Medusa");
                    break;
    ...
    public int getIdTask(Player player) {
        if (player.isPet || player.isDaoLu || player.isTrieuhoipet || player.isBoss || player.playerTask == null || player.playerTask.taskMain == null) {
            return -1;
        }
    ...
    public void changeSideTask(Player player, byte level) {
        if (player.playerTask.sideTask.leftTask > 0) {
            ...
            Service.getInstance().sendThongBao(player, "Bạn đã nhận hết nhiệm vụ hôm nay. Hãy chờ tới ngày mai rồi nhận tiếp");
        }
    }
...
//                Service.getInstance().sendMoney(player);
                Service.getInstance().sendThongBao(player, "Bạn nhận được " + goldReward + " Thỏi vàng");
                player.playerTask.sideTask.reset();
            } else {
    ...
    private void notifyProcessSideTask(Player player) {
        ...
        if (percentDone != 100) {
            ...
            if (notify) {
                Service.getInstance().sendThongBao(player, "Nhiệm vụ: " + player.playerTask.sideTask.getName() + " đã hoàn thành: " + player.playerTask.sideTask.count + "/" + player.playerTask.sideTask.maxCount + " (" + percentDone + "%)");
            }
        } else {
            Service.getInstance().sendThongBao(player, "Chúc mừng bạn đã hoàn thành nhiệm vụ, " + "bây giờ hãy quay về Bò Mộng trả nhiệm vụ.");
        }
    }

    public void nextToTask(Player player, int task260) {
        doneTask(player, task260);
        rewardDoneTask(player);
        player.playerTask.taskMain = TaskService.gI().getTaskMainById(player, task260 + 1);
        sendTaskMain(player);
        Service.gI().sendThongBao(player, "Đã next nhiệm vụ thành công");
    }
}
