...
public class Kaido extends Boss {
    ...
    public Kaido() throws Exception {
        super(BossID.KAIDO, BossesData.KAIDO);
    }
        ...
        if (pl != null) {
            if (!pl.isPl()) {
                ...
                if (pl.fusion.typeFusion == ConstPlayer.NON_FUSION) {
                    if (pl.setClothes.ctHaiTac != -1) {
                        return true;
                    } else {
                        Service.gI().sendThongBao(pl, "Phải mang cải trang hải tặc tí hon mới có thể tấn công Kaido");
                    }
                } else {
    ...
    public void wakeupAnotherBossWhenDisappear() {
        ...
        try {
            for (Boss boss : this.parentBoss.bossAppearTogether[this.parentBoss.currentLevel]) {
                if (boss.id == BossID.LUFFY_GEAR_FIVE_FIGHT_KAIDO && !boss.isDie()) {
                    Service.gI().chat(boss, "Wano Quốc cuối cùng cũng có lại sự yên bình..!");
                    boss.isWin = true;
