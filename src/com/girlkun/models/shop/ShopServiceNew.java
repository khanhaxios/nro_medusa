...
public class ShopServiceNew {
    ...
    public boolean isUsePGGWithType3(Player player, int iconSpecBuy) {
        return InventoryServiceNew.gI().findItemBag(player, 459) != null
                && (iconSpecBuy == 7743);
    }
...
//            player.idmuanhieu = -1;
//            return false;
//        } else if (player.inventory.skMedusa < coupon) {
...
//        player.inventory.skMedusa -= coupon;
        ...
        switch (is.typeSell) {
            ...
            case COST_GEM:
                ...
                if (player.inventory.gem < gem) {
                    Service.getInstance().sendThongBao(player, "Bạn không có đủ ngọc xanh");
                    player.soluongmuanhieu = 0;
                    player.idmuanhieu = -1;
                    return false;
                }
                break;
            case COST_RUBY:
//                int buySpec = (InventoryServiceNew.gI().findItemBag(player, 459) != null && is.iconSpec == 7743) ? is.cost / 2 : is.cost;
                ruby = is.cost;
            ...
            case COST_COUPON:
                coupon = is.cost;
                if (player.inventory.skMedusa < coupon) {
                    Service.getInstance().sendThongBao(player, "Bạn không có đủ điểm");
                    player.soluongmuanhieu = 0;
                    player.idmuanhieu = -1;
                    return false;
            ...
            case COST_EVENT:
                event = is.cost;
                if (player.inventory.event < event) {
                    Service.getInstance().sendThongBao(player, "Bạn không có đủ điểm săn Boss");
                    player.soluongmuanhieu = 0;
                    player.idmuanhieu = -1;
                    return false;
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        if (shop.typeShop == ShopServiceNew.NORMAL_SHOP) {
            ...
            if (!this.subItemByItemShop(player, is)) {
                return;
            }
        }
    ...
    private boolean subItemByItemShop(Player pl, ItemShop itemShop) {
        boolean isBuy = false;
        short itSpec = ItemService.gI().getItemIdByIcon((short) itemShop.iconSpec);
        Item pgg = InventoryServiceNew.gI().findItemBag(pl, 459);
        int buySpec = itemShop.cost;
        //(pgg != null && itemShop.iconSpec == 7743) ? itemShop.cost / 2 : 
        ...
        switch (itS.template.id) {
            ...
            case 861:
                if (itemShop.tabShop.shop.tagName.equals("BILL")) {
                    boolean hasThucAn = false;
                    for (Item i : pl.inventory.itemsBag) {
                        if (i.template != null) {
                            if (ItemData.list_thuc_an.contains((int) i.template.id)) {
                                hasThucAn = true;
                                Item itemThucAns = InventoryServiceNew.gI().findItemBag(pl, i.template.id);
                                if (itemThucAns.quantity < 99) {
                                } else {
                                    if (pl.inventory.ruby >= buySpec) {
                                        pl.inventory.ruby -= buySpec;
                                        InventoryServiceNew.gI().subQuantityItemsBag(pl, itemThucAns, 99);
                    ...
                    if (pl.inventory.ruby >= buySpec) {
                        if (pgg != null && applyPgg) {
                            InventoryServiceNew.gI().subQuantityItemsBag(pl, pgg, 1);
                            Service.getInstance().sendThongBao(pl, "Đã sử dụng phiếu giảm giá");
                            InventoryServiceNew.gI().sendItemBags(pl);
                            if (InventoryServiceNew.gI().findItemBag(pl, 459) == null) {
                                Service.getInstance().sendThongBao(pl, "Túi đã hết phiếu giảm giá, vui lòng mở lại shop để cập nhật giá gốc!");
                            }
                        }
                        pl.inventory.ruby -= buySpec;
                        Service.getInstance().sendMoney(pl);
            ...
            default:
                Item itsCost = InventoryServiceNew.gI().findItemBag(pl, itSpec);
                if (itsCost == null || !InventoryServiceNew.gI().findItemBag(pl, itSpec).isNotNullItem()) {
                    Service.getInstance().sendThongBao(pl, "Không tìm thấy " + itS.template.name);
                    isBuy = false;
                    pl.soluongmuanhieu = 0;
                    pl.idmuanhieu = -1;
                } else if (itsCost.quantity < buySpec) {
                    Service.getInstance().sendThongBao(pl, "Bạn còn thiếu " + (buySpec - itsCost.quantity) + " " + itS.template.name);
                    isBuy = false;
                    pl.soluongmuanhieu = 0;
                    pl.idmuanhieu = -1;
                } else {
                    InventoryServiceNew.gI().subQuantityItemsBag(pl, itsCost, buySpec);
                    isBuy = true;
                }
                break;
        }
