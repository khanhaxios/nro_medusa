...
public class BossDetuBroly extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        int a = 0;
        for (int i = 0; i < 8; i++) {
            ItemMap it1 = new ItemMap(this.zone, 1340, 5, this.location.x + a, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 24), plKill.id);
        ...
        if (Util.isTrue(5, 100)) {
            ItemMap it21 = new ItemMap(this.zone, 542, 1, this.location.x - 10, this.zone.map.yPhysicInTop(this.location.x,
                    this.location.y - 24), plKill.id);
            Service.getInstance().dropItemMap(this.zone, it21);
        }
    }
